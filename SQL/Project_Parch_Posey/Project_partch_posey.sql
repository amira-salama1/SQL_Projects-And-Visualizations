...
with t1 as 
	(Select Top 10 a.id, a.name, sum(o.total_amt_usd) total_spent
	from Project_Parch_posey..accounts a
	join Project_Parch_posey..orders o
	on a.id = o.account_id
...
select t1.id, t1.name, avg(total_spent) Avg_Spent
from t1
...
with t1 as 
	(select Top 1 a.name, sum(o.standard_qty) total_std_qty, SUM(o.total) overall_total
	from Project_Parch_posey..orders o
	join Project_Parch_posey..accounts a
	on a.id = o.account_id
	group by a.name
...
t2 as 
	...
	on a.id = o.account_id
	group by a.name
	having sum(o.total) > (select overall_total from t1))
select  count(*)
from t2;
